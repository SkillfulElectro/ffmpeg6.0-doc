/*
 * Hello everyone welcome to tutorial two
 * in this section we gonna talk about how to save
 * our images in color PPM
*/
...
void save_rgb_frame(unsigned char* buf, int wrap, int xsize, int ysize, const char* filename)
{
    FILE* f;
    int i;
    f = fopen(filename, "wb");
...
int main(int argc, char *argv[])
{
    ...
    /*
     * SwsContext is struct in ffmpeg which is used to scale the image , but why do we need to scale our
     * pgm ?
     *
     *      because pgm is just saving each pixel with one signel 8bit int , for showing its brightness
     *      but ppm is larger and saves each pixel with its rgb , means 3 8bit int
    */
    ...
    */
    ...
    // opening Codec for decoding , the last para is AVDictionary ( for giving options to demuxer )
    ...
    int frameNumber = 0;
    //filling the packet to send it to decoder
    while (av_read_frame(pFormatCtx, pPacket) >= 0) {
        if (pPacket->stream_index == videoStream) {
            ...
            while (response >= 0) {
                ...
                // file addr which i did choose to save the ppm file
                QString fileName {
                    QString("H:/projects_codes/Video_tests/frame_") + QString::number(frameNumber) + QString(".ppm")
                };


                printf(" Frame %d(type = %c, size = %d bytes, format = %d) pts %ld keyFrame %d [DTS %d]",
                       pCodecCtx->frame_num, av_get_picture_type_char(pFrame->pict_type),
                       pFrame->pkt_size, pFrame->format, pFrame->pts, pFrame->key_frame, pFrame->best_effort_timestamp);

                /*
                 * for creating new SwsContext we use sws_getContext
                 *
                 * the parameters it need are :
                 * width of the input video frame
                 * height of the input video frame
                 * the pixel format of the input video frame
                 * The width of the output video frame
                 * The height of the output video frame
                 * The pixel format of the output video frame
                 * A set of flags that control the scaling and conversion process
                 * srcFilter: A filter that is used to sharpen the input video frame.
                 * dstFilter: A filter that is used to sharpen the output video frame.
                 * param: A pointer to a structure that contains additional parameters for the scaling and conversion process
                */
                ...
                // we using this function to calculate the buffer size which is needed for storing the wanted img
                int num_bytes = av_image_get_buffer_size(AV_PIX_FMT_RGB24, pCodecCtx->width, pCodecCtx->height, 1); //https://stackoverflow.com/questions/35678041/what-is-linesize-alignment-meaning

                // allocating memory for our img
                unsigned char* frame_buffer = (uint8_t*)av_malloc(num_bytes);

                /*
                 * in this line of the code we fill our pFrameRGB , using the av_image_fill_arrays
                 * The function returns the size of the image buffer in bytes.
                 * If the function is unable to fill in the data pointers and line sizes,
                 * it returns a negative value
                 *
                 * dst_data: The pFrameRGB->data pointer.
                 * This pointer points to an array of pointers to the image data.
                 * dst_linesizes: The pFrameRGB->linesize pointer.
                 * This pointer points to an array of line sizes for each image plane.
                 * ptr: The frame_buffer pointer. This pointer points to the image buffer.
                 * pix_fmt: The AV_PIX_FMT_RGB24 pixel format.
                 * width: The pCodecCtx->width value. This value represents the width of the image in pixels.
                 * height: The pCodecCtx->height value. This value represents the height of the image in pixels.
                 * align: The value 1. This value indicates that the image buffer should be aligned on a 1-byte boundary.
                */
                ...
                // setting our img height and width
                pFrameRGB->width = pCodecCtx->width;
                pFrameRGB->height = pCodecCtx->height;

                /*
                 * The sws_scale() function in the FFmpeg library is used to scale and convert video frames.
                 * It is a very powerful and versatile function,
                 * and it can be used to perform a wide variety of tasks
                 *
                 * The sws_scale() function takes the following arguments:
                 *
                 * c: A pointer to a SwsContext structure.
                 * srcSlice: An array of pointers to the source video frame data.
                 * srcStride: An array of line strides for the source video frame data.
                 * srcSliceY: The vertical position of the source video frame slice to scale.
                 * srcSliceH: The height of the source video frame slice to scale.
                 * dst: An array of pointers to the destination video frame data.
                 * dstStride: An array of line strides for the destination video frame data.
                 *
                 * The function returns the height of the scaled video frame in pixels.
                 * If the function is unable to scale the video frame, it returns a negative value
                */
                ...
                {
                    printf("\nMaybe not grayscale generated\n");
                }

                /*
                 * i kept pFrame like how it is , to be able to use save_gray_frame too
                */
                //save_gray_frame(pFrame->data[0] , pFrame->linesize[0] , pFrame->width , pFrame->height , fileName.toStdString().c_str());
                save_rgb_frame(pFrameRGB->data[0], pFrameRGB->linesize[0], pFrameRGB->width, pFrameRGB->height, fileName.toStdString().c_str());
...
//snprintf(frame_filename, sizeof(frame_filename), "%s-%d.ppm","frame", pCodecCtx->frame_num);
...
*/
/*
int ppm_to_png(const char *input_file_path, const char *output_file_path) {
    // Open the input PPM file.
    AVFormatContext *pFormatCtx = avformat_alloc_context();
    ...
    // Allocate a SWS context for scaling the video.
    ...
    // Decode the input PPM file and write the decoded video to a PNG file.
    AVPacket packet;
    while (av_read_frame(pFormatCtx, &packet) >= 0) {
        if (packet.stream_index == videoStream) {
            avcodec_decode_video2(pCodecCtx, pFrame, NULL, &packet);

            // Scale the decoded video to the desired output size.
            uint8_t *outputBuffer = (uint8_t *)malloc(pCodecCtx->width * pCodecCtx->height * 3);
            if (outputBuffer == NULL) {
                return -1;
            }
            sws_scale(swsCtx, pFrame->data, pFrame->linesize, 0, pCodecCtx->height, &outputBuffer, NULL);
            ...
            // Write the PNG header to the output file.
            fwrite(PNG_HEADER, 1, sizeof(PNG_HEADER), output);

            // Write the scaled video data to the output file.
            for (int i = 0; i < pCodecCtx->height; i++) {
                fwrite(outputBuffer + i * pCodecCtx->width * 3, 1, pCodecCtx->width * 3, output);
            }

            // Write the PNG footer to the output file.
            fwrite(PNG_FOOTER, 1, sizeof(PNG_FOOTER), output);

            // Close the output PNG file.
            fclose(output);
